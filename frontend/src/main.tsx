...
type SearchFilters = { query:string; roles:CatalogRole[]; remoteModes:string[]; employmentTypes:string[]; countryCode:string; place:string; minimumMonthlySalary:string; publishedWithinDays:string; excludeEmployers:boolean };
type SavedSearch = { id:string; name:string; version:number; criteria:{ query?:string; roleFamilyIds?:string[]; remoteModes?:string[]; employmentTypes?:string[]; countryCode?:string; place?:string; state?:string; city?:string; minimumMonthlySalary?:number; publishedWithinDays?:number; excludeEmployers?:boolean } };
...
const emptySearchFilters = (): SearchFilters => ({ query:'', roles:[], remoteModes:[], employmentTypes:[], countryCode:'', place:'', minimumMonthlySalary:'', publishedWithinDays:'', excludeEmployers:false });
...
function ExploreView({ notify }: { notify:(value:string)=>void }) {
  ...
  async function fetchPage(active:SearchFilters,cursor?:string){
    ...
    if(active.place.trim())params.set('place',active.place.trim());
  ...
  async function applySaved(item:SavedSearch){const c=item.criteria;const next:SearchFilters={query:c.query||'',roles:(c.roleFamilyIds||[]).map(id=>catalogRoles.find(role=>role.id===id)||{id,name:'Rol guardado'}),remoteModes:c.remoteModes||[],employmentTypes:c.employmentTypes||[],countryCode:c.countryCode||'',place:c.place||c.city||c.state||'',minimumMonthlySalary:c.minimumMonthlySalary?.toString()||'',publishedWithinDays:c.publishedWithinDays?.toString()||'',excludeEmployers:Boolean(c.excludeEmployers)};setSearchDisplay(item.name);setAppliedSavedSearchName(item.name);setSavedSearchOpen(false);setFilters(next);await search(next);}
...
function criteria(filters:SearchFilters){return {query:filters.query.trim()||null,roleFamilyIds:filters.roles.map(role=>role.id),remoteModes:filters.remoteModes,employmentTypes:filters.employmentTypes,countryCode:filters.countryCode.trim()||null,place:filters.place.trim()||null,minimumMonthlySalary:filters.minimumMonthlySalary?Number(filters.minimumMonthlySalary):null,publishedWithinDays:filters.publishedWithinDays?Number(filters.publishedWithinDays):null,excludeEmployers:filters.excludeEmployers};}
