...
function ReasonGroup({type,label,icon,reasons}:{type:'MATCH'|'GAP'|'CONSIDERATION';label:string;icon:string;reasons:Match['reasons']}){const visible=reasons.slice(0,3);const hidden=reasons.slice(3);return <section className={`reason-group ${type.toLowerCase()}`}><h4><span>{icon}</span>{label}</h4><ul>{visible.map(reason=><Reason key={reason.id} reason={reason}/>)}</ul>{hidden.length>0&&<details><summary>Mostrar {hidden.length} {hidden.length===1?'razón más':'razones más'}</summary><ul>{hidden.map(reason=><Reason key={reason.id} reason={reason}/>)}</ul></details>}</section>;}
function Reason({reason}:{reason:Match['reasons'][number]}){const evidence=reasonEvidence(reason.evidence);return <li><span>{reason.explanation}</span>{evidence&&<small>{evidence}</small>}</li>;}
function reasonEvidence(evidence?:Record<string,unknown>){if(!evidence)return '';if(typeof evidence.targetRole==='string'&&typeof evidence.priority==='number')return `Evidencia: rol objetivo “${evidence.targetRole}”, prioridad ${evidence.priority}.`;if(typeof evidence.language==='string'&&typeof evidence.proficiency==='string')return `Evidencia: ${evidence.language}, nivel ${evidence.proficiency}.`;if(typeof evidence.profileSeniority==='string')return `Evidencia: seniority ${evidence.profileSeniority}.`;const parts:string[]=[];if(typeof evidence.skill==='string')parts.push(evidence.skill);if(typeof evidence.professionalMonths==='number'&&evidence.professionalMonths>0)parts.push(`${evidence.professionalMonths} meses profesionales`);if(typeof evidence.weightedPracticalMonths==='number'&&evidence.weightedPracticalMonths>0)parts.push(`${Number(evidence.weightedPracticalMonths)} meses en prácticas o proyectos`);if(Array.isArray(evidence.trajectoryIds)&&evidence.trajectoryIds.length)parts.push(`${evidence.trajectoryIds.length} elemento${evidence.trajectoryIds.length===1?'':'s'} de trayectoria`);return parts.length?`Evidencia: ${parts.join(' · ')}.`:'';}
